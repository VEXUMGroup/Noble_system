...
export default function ContractDetailPage({ params }: ContractDetailPageProps) {
  ...
  const [planCode, setPlanCode] = useState(deal?.plan_code || '');
  const [amount, setAmount] = useState(deal?.amount?.toString() || '');
  const [paymentMethod, setPaymentMethod] = useState(deal?.payment_method || '');
  const [paymentDeadline, setPaymentDeadline] = useState(deal?.payment_deadline || '');
  ...
  const handleConfirm = () => {
    ...
    if (!planCode || Number.isNaN(parsedAmount) || parsedAmount <= 0 || !paymentMethod || !paymentDeadline) {
      setErrorMessage('必須項目をすべて正しく入力してください。');
      return;
    }

    deal.plan_code = planCode;
