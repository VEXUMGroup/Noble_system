...
import { buildNextActionTargets, type NextActionDeal } from '@/lib/next-action-notifications';
import {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: deals, error: dealsError } = await supabase
      .from('deals')
      .select('id, customer_name, assigned_to, status, next_action_date')
      .eq('next_action_date', todayYmd);

    if (dealsError) {
      return NextResponse.json({ error: dealsError.message }, { status: 500 });
    }

    const targets = buildNextActionTargets((deals ?? []) as NextActionDeal[]);
    ...
    for (const target of targets) {
      ...
      const message = `${target.customerName} 様の次回アクション日が本日です。`;
