...
export async function GET() {
  try {
    ...
    if (error) {
      console.error('[ical-url GET] error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    return NextResponse.json({ ical_url: data?.ical_url ?? null });
  } catch (err) {
    console.error('[ical-url GET] unhandled:', err);
...
export async function PUT(req: NextRequest) {
  try {
    ...
    console.error('[ical-url PUT] unhandled:', err);
    const message = err instanceof Error ? err.message : 'internal_error';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
...
export async function DELETE() {
  try {
    ...
    if (error) {
      console.error('[ical-url DELETE] error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error('[ical-url DELETE] unhandled:', err);
    const message = err instanceof Error ? err.message : 'internal_error';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
